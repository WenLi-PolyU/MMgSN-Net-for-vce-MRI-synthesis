...
class Multi_modal_generator(nn.Module):
    ...
    def __init__(self,input_nc, output_nc, ngf):
        ...
        #self.attn1_0    = Self_Attn(self.out_dim, act_fn)
        ...
        self.down_3_0 = nn.Sequential(
                nn.Conv2d(in_channels=self.out_dim*8,  out_channels=self.out_dim*4, kernel_size=3, stride=1, padding=1),nn.BatchNorm2d(self.out_dim*4), act_fn, #
        ...
        #self.attn3_0  = Self_Attn(self.out_dim*4, act_fn)
        ...
        #self.attn1_1    = Self_Attn(self.out_dim, act_fn)
        ...
        #self.attn3_1  = Self_Attn(self.out_dim*4, act_fn)
        ...
        #self.attn_fu_2m = Self_Attn(self.out_dim*4, act_fn)
        ...
        #self.attn_deconv_2_0 = Self_Attn(self.out_dim*2, act_fn2) #~~~~-_-_-_-_-~~~~~~~~
        self.deconv_3_0 = MixedFusion_Block(self.out_dim * 2, self.out_dim * 1,act_fn2)
        self.attn_deconv_3_0 = Self_Attn(self.out_dim*1, act_fn2) #~~~~-_-_-_-_-~~~~~~~~
        self.deconv_4_0 = MixedFusion_Block(self.out_dim * 1, self.out_dim,act_fn2)  
        #self.attn_deconv_4_0 = Self_Attn(self.out_dim*1, act_fn2) #~~~~-_-_-_-_-~~~~~~~~
        self.deconv_5_0 = conv_decod_block(self.out_dim * 1, int(self.out_dim*1),act_fn2) 
        #self.attn_deconv_5_0 = Self_Attn(int(self.out_dim*(1/2)), act_fn2) #~~~~-_-_-_-_-~~~~~~~~
        #self.out        = nn.Sequential(nn.Conv2d(int(self.out_dim*9),1, kernel_size=1, stride=1, padding=0),nn.Tanh())
        self.out        = nn.Sequential(nn.Conv2d(int(self.out_dim*9),1, kernel_size=1, stride=1, padding=0),nn.Tanh())
    ...
    def forward(self,inputs):
        ...
        # ############################# #
        i0 = inputs[:,0:1,:,:] #（1,1,256,224）
        i1 = inputs[:,1:2,:,:] #（1,1,256,224）
        
        # -----  First Level -------- 
        down_1_0 = self.down_1_0(i0) #（1,32,256,224）
        down_1_1 = self.down_1_1(i1) #（1,32,256,224）
        

        # -----  Second Level --------
        # Max-pool
        down_1_0m   = self.pool_1_0(down_1_0) #（1,32,128,112）
        down_1_1m   = self.pool_1_1(down_1_1) #（1,32,128,112）
        
        #down_1_0m   = self.attn1_0(down_1_0m)
        #down_1_1m   = self.attn1_1(down_1_1m)
        #
        down_fu_1   = self.down_fu_1(down_1_0,down_1_1) #（1,64,256,224）                                                                                               
        down_fu_1m  = self.pool_fu_1(down_fu_1) #（1,64,128,112） 
        down_fu_1m_2  = self.pool_fu_1(down_fu_1m) #(1,64,64,56) 
        
        down_2_0 = self.down_2_0(torch.cat((down_1_0m,down_fu_1m),dim=1))  #（1,64,128,112）  
        down_2_1 = self.down_2_1(torch.cat((down_1_1m,down_fu_1m),dim=1))  #（1,64,128,112）  
        
        
        # -----  Third Level --------
        # Max-pool
        down_2_0m = self.pool_2_0(down_2_0)  #（1,64,64,56）
        down_2_1m = self.pool_2_1(down_2_1)  #（1,64,64,56）
        #
        down_fu_2   = self.down_fu_2(down_2_0,down_2_1,down_fu_1m)  #（1,128,128,112）                                                                                  
        down_fu_2m  = self.pool_fu_2(down_fu_2)  #（1,128,64,56） 
        ...
        down_fu_3   = self.down_fu_3(down_3_0,down_3_1,down_fu_2m) #（1,128,64,56） 
        down_fu_4   = self.down_fu_4(down_fu_3) #（1,256,64,56） 
        

        #######################################################################                                                                                                
        # ~~~~~~ Decoding 
        deconv_1_0 = self.deconv_1_0(down_fu_4) #（1,128,64,56） 
        deconv_2_0 = self.deconv_2_0(down_3_0,down_3_1,deconv_1_0) #（1,64,64,56）      
        #deconv_2_0 = self.attn_deconv_2_0(deconv_2_0)
        deconv_3_0 = self.deconv_3_0(down_2_0,down_2_1,up(deconv_2_0)) #（1,32,128,112）   
        deconv_3_0 = self.attn_deconv_3_0(deconv_3_0) 
        deconv_4_0 = self.deconv_4_0(down_1_0,down_1_1,up(deconv_3_0)) #（1,32,256,224）   
        #deconv_4_0 = self.attn_deconv_4_0(deconv_4_0)     
        deconv_5_0 = self.deconv_5_0(deconv_4_0) #（1,32,256,224）  
        #deconv_5_0 = self.attn_deconv_5_0(deconv_5_0)       
        
        scale1 = _upsample_like(deconv_1_0,deconv_5_0)
        scale2 = _upsample_like(deconv_2_0,deconv_5_0)
        scale3 = _upsample_like(deconv_3_0,deconv_5_0)
        scale4 = deconv_4_0
        
        output       = self.out(torch.cat((deconv_5_0,scale1,scale2,scale3,scale4),1))
...
class Discriminator(nn.Module):
    def __init__(self, in_channels=1):
        ...
        self.model = nn.Sequential(
            *discrimintor_block(in_channels, 32, normalize=True),#changed
